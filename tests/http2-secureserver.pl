...
my $logfile = "log/http2secure.log";
my $nghttpx = "nghttpx";
my $listenport = 9018;
...
while(@ARGV) {
    if($ARGV[0] eq '--verbose') {
        ...
        print STDERR "\nWarning: http2-secureserver.pl unknown parameter: $ARGV[0]\n";
    }
    shift @ARGV;
}

my $certfile = "$srcdir/certs/Server-localhost-sv.crt";
my $keyfile = "$srcdir/certs/Server-localhost-sv.key";
